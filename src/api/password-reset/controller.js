...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your kintos-api account.<br>
        ...
        &mdash; kintos-api Team
      `
      return sendMail({ toEmail: email, subject: 'kintos-api - Password Reset', content })
    })
